...
import { AdminDeletePostUseCase } from './admin-delete-post.use-case';
...
describe('AdminDeletePostUseCase', () => {
  let useCase: AdminDeletePostUseCase;
  ...
  beforeEach(async () => {
    ...
    useCase = module.get(AdminDeletePostUseCase);
    postRepository = module.get(POST_REPOSITORY);
  });

  // Owner should delete the post
  it('should delete the post when it belongs to the admin', async () => {
    ...
    await useCase.execute(postId, adminId);
  ...
  // Non-owner should be rejected and delete must not run
  it('should throw ForbiddenException when the post belongs to another admin', async () => {
    ...
    await expect(useCase.execute(postId, otherAdminId)).rejects.toThrow(
      new ForbiddenException('No tienes permiso para eliminar este post'),
    );
    expect(postRepository.delete).not.toHaveBeenCalled();
  });
});
